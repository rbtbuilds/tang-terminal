...
Copy `terminal/.tang-terminal.env.example` to `.tang-terminal.env` only when testing optional local credentials such as AISStream, Finnhub, or Twelve Data. Dedicated ignored provider files are also supported. Never commit a configured file.
